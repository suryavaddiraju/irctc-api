...
import { URL } from 'url';
import {verify_booking_params, initialize_booking_variables, input, base64DecodeUnicode,verify_txn_params, initialize_txn_variables} from './utils.mjs';
import { sleep, sleep_for_login, sleep_for_availability_check} from "./time_work.mjs";
...
class IRCTC{
    ...
    async paytm_pg_landing_page(){
        const $ = cheerio.load(this.paytm_params_d);
        const script_tags = $('script').map((idx, el) => $(el).html()).toArray();
        let script_data = "";
        for (const script of script_tags) {
            if (script.includes('pushAppData="')){
                script_data += script;
            }
        }
